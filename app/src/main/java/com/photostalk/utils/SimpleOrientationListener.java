...
public abstract class SimpleOrientationListener extends OrientationEventListener {
    ...
    public void onOrientationChanged(final int orientation) {
        ...
        if (orientation >= 330 || orientation < 30) {
            ...
            Log.d("SimpleOrientation", "Rotation 0");
        } else if (orientation >= 60 && orientation < 120) {
            Log.d("SimpleOrientation", "Rotation 90");
            currentOrientation = Surface.ROTATION_90;
        } else if (orientation >= 150 && orientation < 210) {
            Log.d("SimpleOrientation", "Rotation 180");
            currentOrientation = Surface.ROTATION_180;
        } else if (orientation >= 240 && orientation < 300) {
            Log.d("SimpleOrientation", "Rotation 270");
    ...
    private void reportOrientationChanged(final int currentOrientation) {
        ...
        if (currentOrientation == Surface.ROTATION_0 || currentOrientation == Surface.ROTATION_180)
            toReportOrientation = ROTATION_0;
        else if (currentOrientation == Surface.ROTATION_90)
            toReportOrientation = ROTATION_90;
        else
            toReportOrientation = ROTATION_270;
